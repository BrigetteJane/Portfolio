...
http.open('GET','uiuxDesign.json', true);
...
http.onload = function(){
  if (this.readyState ==4 && this.status==200){
    let uiuxProjects = JSON.parse(this.responseText);
    let output="";

    for (let item of uiuxProjects){
      output += `
    ...
    document.querySelector('.uiuxProjects').innerHTML = output;
  }
}
